...
const DrawingCanvas = () => {
  ...
  const getPixelPos = (x, y) => {
    return (y * context.current.canvas.width + x) * 4;
  };

  const matchStartColor = (data, pixelPos, startColor) => {
    return (
      data[pixelPos] === startColor.r &&
  ...
  const colorPixel = (data, pixelPos, color) => {
    data[pixelPos] = color.r || 0;
    data[pixelPos + 1] = color.g || 0;
    data[pixelPos + 2] = color.b || 0;
    data[pixelPos + 3] = color.hasOwnProperty('a') ? color.a : 255;
  };

  const handleFill = (x, y, color) => {
    let img = context.current.getImageData(0, 0, context.current.canvas.width, context.current.canvas.height);
    let data = img.data;
    let start = getPixelPos(x, y);
    let startColor = { r: data[start], g: data[start + 1], b: data[start + 2], a: data[start + 3] };
    let pixelStack = [[x, y]];

    while (pixelStack.length) {
      let pixelPos = pixelStack.pop();
      let pixelX = pixelPos[0];
      let pixelY = pixelPos[1];
      let currentPos = getPixelPos(pixelX, pixelY);

      while (pixelY-- >= 0 && matchStartColor(data, currentPos, startColor)) {
        currentPos -= context.current.canvas.width * 4;
      }
      ...
      while (pixelY++ < context.current.canvas.height - 1 && matchStartColor(data, currentPos, startColor)) {
        colorPixel(data, currentPos, color);

        if (pixelX > 0) {
          if (matchStartColor(data, currentPos - 4, startColor)) {
            if (!reachLeft) {
              pixelStack.push([pixelX - 1, pixelY]);
              reachLeft = true;
            }
          } else if (reachLeft) {
            reachLeft = false;
        ...
        if (pixelX < context.current.canvas.width - 1) {
          if (matchStartColor(data, currentPos + 4, startColor)) {
            if (!reachRight) {
              pixelStack.push([pixelX + 1], pixelY);
              reachRight = true;
    ...
    context.current.putImageData(img, 0, 0);
  };
  ...
  const onMouseDown = (event) => {
    ...
    if (activeTool === 'fill') {
      handleFill(event.clientX - boundingRect.left, event.clientY - boundingRect.top, strokeStyle);
    }
  };
